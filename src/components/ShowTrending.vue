  ...
  <div>
    <div v-if="show && isTrending" class="relative m-1 w-60 lg:w-max lg:m-0">
        ...
        <router-link to="/videos">
            <div v-if="store.showOverlay === show.id" class="absolute inset-0 flex items-center z-50 w-fit m-auto">
                <p class="text-white flex gap-2 items-center bg-white bg-opacity-30 p-2 rounded-full cursor-pointer lg:text-2xl lg:gap-4">
          ...
          <div class="absolute inset-0 bg-black bg-opacity-35 flex items-end justify-center rounded-xl w-11/12">
            <div class="w-11/12 px-2">
              ...
              <h2 class="font-bold text-lg lg:text-2xl">{{ show.original_title || show.original_name }}</h2>
            </div>
